...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		// $multipage .= '<a class="dots">'.$num.L('page_item').'</a>';
		if($curr_page>0) {
		...
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				...
				$multipage .= ' <a href="javascript:void(0)" >'.$i.'</a>';
			}
		}
		if($curr_page<$pages) {
			...
			$multipage .= ' <a href="javascript:void(0)">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page, $array).'">'.L('next').'</a>';
		} else {
			$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" >'.L('next').'</a>';
		}
	}
